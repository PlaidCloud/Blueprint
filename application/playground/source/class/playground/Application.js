/* ************************************************************************
   ...
   Modified by Dan Hummon for use with Tartan Blueprint.
  ...
  members :
  {
    ...
    */
    ...
    updatePlayground : function(root)
    {
      ...
      try
      {
        this.json = qx.util.Json.parseQx(this.code);
        this.__playApp.getRoot().add(blueprint.Manager.getInstance().generate(this.json, null, "play_app", false));
    ...
    __createSampleMenu : function()
    {
      ...
      {
        if (elem[i].className == "blueprint_samples")
        {
   ...
   __toggleEditor : function(e)
   {
    ...
    __createToolbar : function()
    {
      ...
      runButton.setToolTipText(this.tr("Run the serialization code"));
